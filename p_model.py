...
########################################################
"""
此模块用于定义神经网络，该网络为输入，lstm，中间为两层输入加lstm，最后为池化和输出层
"""
...
def db_lstm(data_reader, word, postag, conf_dict):
    ...
    和最大池化
    """
    hidden_dim = conf_dict['hidden_dim']#定义隐藏层
    depth = conf_dict['depth']#定义深度
    label_dict_len = data_reader.get_dict_size('label_dict')#标签字典长度
    ...
    hidden_0 = fluid.layers.sums(input=hidden_0_layers)#输入层
    ...
    lstm_0 = fluid.layers.dynamic_lstm(
        ...
        cell_activation='sigmoid')#定义lstm单元

    # 用直边堆叠L-LSTM和R-LSTM
    input_tmp = [hidden_0, lstm_0]#合二为一
    ...
    for i in range(1, depth):
        ...
        lstm = fluid.layers.dynamic_lstm(
            ...
            is_reverse=((i % 2) == 1))#隐藏层后面的lstm层

        input_tmp = [mix_hidden, lstm]#合二为一并重新定义
